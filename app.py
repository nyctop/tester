...
def run_instaloader():
    ...
    command = f"instaloader profile {username}"
    
    stream = os.popen(command)
    output = stream.read()
    
    if "QueryReturnedNotFoundException" in output or "JSONDecodeError" in output:
        return f"Instaloader çalıştırılamadı. Hata: {output} <a href='/'>Geri Dön</a>"
